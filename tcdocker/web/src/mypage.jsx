...
import { S3URL } from "./axioses/config"
...
export default function Mypage() {
    ...
    useEffect(() => {
        ...
        if (id) {
            const UserDetail = axios.get(`/users/${id}`);
            const FeedList = axios.get(`/feeds/users/${id}`);
        ...
        if (bcid) {
            const donComList = axios.get(`/transactions/donations/${bcid}`);
            axios
                .all([donComList])
